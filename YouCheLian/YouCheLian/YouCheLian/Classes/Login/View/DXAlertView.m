...
typedef enum : NSUInteger {
    DXAlertViewBlockLeft =0,
    DXAlertViewBlockRight,
    DXAlertViewBlockDismiss,
} DXAlertViewBlockType;
...
@property (nonatomic , assign) DXAlertViewBlockType type;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.type = DXAlertViewBlockDismiss;
    }
    return self;
}
...
{
    _leftLeave = YES;
    self.type = DXAlertViewBlockLeft;
...
{
    _leftLeave = NO;
    self.type = DXAlertViewBlockRight;
    [self removeFromSuperview];
...
- (void)removeFromSuperview
{
    ...
    [UIView animateWithDuration:0.35f delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        switch (self.type) {
#warning MIKEWHY 必须要等到改界面完全删除时  才去执行block 否则会发送内存泄露  对象先释放的闪退
            case DXAlertViewBlockDismiss:
            {
