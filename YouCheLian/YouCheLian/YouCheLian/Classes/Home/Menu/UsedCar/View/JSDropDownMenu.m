...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (leftTableView) {
            ...
            #pragma mark 下拉菜单高度设置
            if (self.currentSelectedMenudIndex == 2) {
                // 因为有索引列没计算高度  -80
                leftTableViewHeight = (_JSArrayData.count * leftTableView.rowHeight > (kUIScreenHeight -64-kHeadTitleHeight -80)) ? (kUIScreenHeight -64-kHeadTitleHeight) : _JSArrayData.count * leftTableView.rowHeight;
                ...
                leftTableViewHeight = ([leftTableView numberOfRowsInSection:0] *leftTableView.rowHeight > (kUIScreenHeight -64-kHeadTitleHeight)) ? (kUIScreenHeight -64-kHeadTitleHeight) : ([leftTableView numberOfRowsInSection:0] * leftTableView.rowHeight);
            }
        }
...
//        CGFloat tableViewHeight = MAX(leftTableViewHeight, rightTableViewHeight);
        CGFloat tableViewHeight = MAX(leftTableViewHeight, 0);
...
- (void)initData {
    //init
    _JSArrayData = [[NSMutableArray alloc] init];
    _sortedArrForArrays = [[NSMutableArray alloc] init];
    _sectionHeadsKeys = [[NSMutableArray alloc] init];
    
}

-(void)setJSArrayData:(NSArray *)JSArrayData {
    //    _JSArrayData = JSArrayData;
    
    // 创建数据用来装模型数据
    NSMutableArray *apps = [NSMutableArray array];
    for (NSDictionary *dict in JSArrayData) {
        // 将字典转化模型
        MyModel *model = [MyModel appWithDict:dict];
        [apps addObject:model];
    }
    _JSArrayData = apps;
    
    NSDictionary *dict = [WWPinyinGroup group:apps key:@"name"];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    if (self.currentSelectedMenudIndex == 2) {
        return 1 + [self.sortedArrForArrays count];
    }else {
        return 1;
    }
}
...
- (NSArray *)sectionIndexTitlesForTableView:(UITableView *)tableView {
    if (self.currentSelectedMenudIndex == 2) {
        ...
        mArray =  [NSMutableArray arrayWithArray:self.sectionHeadsKeys];
        [mArray insertObject:@"全部" atIndex:0];
        return mArray;
    }
    return nil;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.currentSelectedMenudIndex == 2) {
        if (indexPath.section == 0) {
            FiltrateCell *cell = [FiltrateCell cellWithTableView:tableView];
        ...
        FiltrateCell *cell = [FiltrateCell cellWithTableView:tableView];
        cell.model = self.sortedArrForArrays[indexPath.section -1][indexPath.row];
        return cell;
        ...
        if (leftOrRight == 1) {
            ...
            titleLabel.frame = CGRectMake(30, 0, textSize.width, cell.frame.size.height);
            ...
            if (!_hadSelected && _leftSelectedRow == indexPath.row) {
                ...
                if(!haveRightTableView) {
                    ...
                    accessoryImageView.frame = CGRectMake(kUIScreenWidth - 50, (self.frame.size.height-12)/2, 16, 12);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:strID:)]) {
        ...
        [self.delegate menu:self didSelectRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:indexPath.row] strID:self.strID];
...
- (void)confiMenuWithSelectRow:(NSInteger)row leftOrRight:(NSInteger)leftOrRight{
    ...
    if (self.currentSelectedMenudIndex == 2) {
        ...
        if (self.indexPath.section == 0) {
            ...
            model = self.sortedArrForArrays[self.indexPath.section -1][self.indexPath.row];
            title.string = model.name;
            self.strID = model.ID;
        }
        
        // 只把数据反回去 不接返回值
        [self.dataSource menu:self titleForRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:leftOrRight leftRow:_leftSelectedRow row:row]];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:strID:)]) {
        ...
        [self.delegate menu:self didSelectRowAtIndexPath:[JSIndexPath indexPathWithCol:self.currentSelectedMenudIndex leftOrRight:-1 leftRow:-1 row:indexPath.row] strID:self.strID];
    } else {
        //TODO: delegate is nil
    }
}
