...
#import "QRView.h"
#import "QRUtil.h"
@interface AddDeviceController ()<AVCaptureMetadataOutputObjectsDelegate,QRViewDelegate>
...
- (void)updateLayout {
    ...
    CGRect cropRect = CGRectMake((kUIScreenWidth - self.qrView.transparentArea.width) / 2,
                                 (kUIScreenHeight - self.qrView.transparentArea.height) / 2,
                                 self.qrView.transparentArea.width,
                                 self.qrView.transparentArea.height);
    
    [_output setRectOfInterest:CGRectMake(cropRect.origin.y / kUIScreenHeight,
                                          cropRect.origin.x / kUIScreenWidth,
                                          cropRect.size.height / kUIScreenHeight,
                                          cropRect.size.width / kUIScreenWidth)];
}
